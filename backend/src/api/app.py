...
def create_app() -> FastAPI:
    ...
    async def startup_event():
        ...
        asyncio.create_task(_emotion_monitor_loop(emotion_detector))
        asyncio.create_task(_timer_loop())

    async def _emotion_monitor_loop(detector: EmotionDetector):
        while True:
            if robot_state.snapshot()["mode"] == RobotMode.ACTIVE:
                emotion = detector.predict_from_frame(None)
    ...
    def handle_command(request: CommandRequest):
        ...
        if snapshot["mode"] == RobotMode.IDLE:
            if wake_detector.is_wake_command(text):
                ...
                response = "Are you sure you would like to power Juno on? Answer yes if you do, else ignore."
